...
def train_epoch(model, epoch):
    ...
    for batch_idx, data in tqdm(enumerate(train_loader)):
        ...
        # CRITICAL: Add noise to last few timesteps during training
        # This prevents the model from simply copying the last value
        if model.training:
            noise_std = 0.05  # 5% noise in scaled space
            last_steps = 3   # Add noise to last 3 timesteps
            noise = torch.randn(b, n_st, last_steps, f, device=device) * noise_std
            historical_data[:, :, -last_steps:, :] = historical_data[:, :, -last_steps:, :] + noise
